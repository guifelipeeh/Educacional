...
const cursoAtualizacaoSchema = Joi.object({
...
const validarCursoAtualizacao = async (req, res, next) => {
  try {
    // Validar os campos do curso
    const { error } = cursoAtualizacaoSchema.validate(req.body);
    if (error) {
    ...
    // Prosseguir para o próximo middleware se os campos estiverem válidos e o curso e o professor existirem
    next();
  } catch (error) {
...
module.exports = validarCursoAtualizacao;
